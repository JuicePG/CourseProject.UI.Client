...
//Data and time script end

getIndicators();

function getIndicators(){
    axios.get('https://localhost:7208/api/Indicator')
    .then(result =>{
        console.log(result);
...
function createIndicator(id, name, x, y, value, unit, values = [])
{
    ...
    diagramButton.onmousedown = onShowDiagramClick;
...
function tryNewIndicator(name, value, unit, x, y)
{
    ...
    axios.post('https://localhost:7208/api/Indicator',{
...
function changeBackgroundImage(id)
{
    ...
    element.src = 'https://localhost:7208/api/BackgroundImage/' + 4;
    console.log(element)
}
...
document.getElementById("imageForm").addEventListener("submit", function(event){
    ...
    if(fileInput)
    {
        ...
        axios.post('https://localhost:7208/api/backgroundimage/upload-image', formData)
        .then(result =>{
...
function onDelete(e, id){
    axios.delete(`https://localhost:7208/api/indicator/${id}`);
